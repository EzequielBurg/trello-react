...
class Panel extends Component {
    ...
    render() {
        ...
        const filteredCards = panel.cards.map(id => cards.find(card => card.id === id)).filter(card => card)

        return connectDragPreview(
            connectDropTarget(
                <div className="col-md-3">
                    { connectDragSource(
                        <div className="panel panel-default panel-general">
                            <div className="panel-heading input-header">
                                <InputEditable
                                    id={ panel.id }
                                    edit={ panel.edit }
                                    text={ panel.text }
                            ...
                            <div className="panel-body">
                                <Cards
                                    ...
                                    deleteComponent={ this.handleDeleteCard }
                                    moveCard={ this.props.moveCard }
                                />
                            </div>
                            <div className="panel-footer">
                                <button className="btn btn card-button" onClick={this.handleCreateCard}>
                                    <i className="ion-plus-round"></i> Task
                                </button>
                            </div>
                        </div>
                    ) }
